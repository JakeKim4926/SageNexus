...
{
public:
    BOOL Export(const DataTable& table, const CString& strFilePath, CString& strError);

private:
    BOOL WriteHtmlFile(const CString& strPath, const std::wstring& content, CString& strError) const;

    std::wstring BuildHtml(const DataTable& table) const;
    std::wstring BuildHeaderRow(const DataTable& table) const;
