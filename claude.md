...
확장 구조를 검토할 때는 아래처럼 나눠 판단한다.

### 지금 유지할 것
- `코어 + DLL 플러그인 + SolutionProfile` 큰 축
- 고객사 전용 기능의 private 플러그인 격리
- DLL 드랍 기반 고객사 배포
- 고객사별 독립 패키지 생성 흐름

### 지금 바로 정리할 것
- 문서와 실제 구현의 정합성
- `README`, ADR, skill의 Phase/상태 날짜 일치 여부
- `profile.json` 설명과 실제 로드/저장 방식 일치 여부
- Settings 변경사항이 메모리 상태인지 영속 저장인지 명시

### 다음 단계에서 리팩터링할 것
- 현재 `IPlugin`은 메타데이터/로드 경계에 가깝다
- 실제 기능 실행 확장이 필요해지면 capability 단위 인터페이스를 검토한다
- 예: `IImportPlugin`, `IExportPlugin`, `IActionPlugin`, `ISchedulerPlugin`
- 고객사 수 증가로 `vcxproj` 구성 추가 비용이 커지면 패키징 메타데이터 중심 구조를 검토한다

즉 현재 단계에서는 구조를 뒤집기보다, 문서-구현 정합성을 먼저 맞추고 그 다음 Phase에서 플러그인 capability를 확장한다.
