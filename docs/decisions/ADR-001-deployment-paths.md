...
- 앱 실행 시 설치 폴더에서 로드한다.
- 파일이 없으면 임베딩된 기본 profile 리소스를 로드한 뒤 설치 폴더에 생성하려고 시도한다.
- Settings 페이지에서 플러그인 on/off를 변경하면 설치 폴더의 `profile.json`에 저장한다.
- 설치 폴더에 쓰기 권한이 없으면 저장은 실패하고, UI는 디스크 기준 상태로 되돌린다.
